...
import { ConsumptionInterface, ConsumptionModel } from '../models/ConsumptionModel';
import moment from 'moment';
import { UserInterface } from '../models/UserModel';

class IndexService {
  static async addIndex(indexValue: number, date: string, user?: UserInterface): Promise<boolean> {
    try {
      // Girilen tarihten büyük/eşit ise ve girilen endeksden küçük/eşit ise
      const greaterIndexes = await IndexModel.find({
        userId: user?._id,
        date: { $gte: date },
        indexValue: { $lte: indexValue },
      });
      if (greaterIndexes.length > 0) {
        ...
        await IndexModel.deleteMany({
          userId: user?._id,
          date: { $in: greaterIndexes.map((index) => index.date) },
        });
  ...
  static async calculateConsumption(user?: UserInterface): Promise<boolean> {
    try {
      // * En son eklenen endeksin tarihini al
      const date = (await IndexModel.find({ userId: user?._id }).sort({ date: -1 }).limit(1))[0]
        .date;

      // * Tarih filtresi ile tüm endeksleri getir, tarihe göre sırala
      const allIndexes = await IndexModel.find({ userId: user?._id, date: { $lte: date } }).sort({
        date: 1,
      });

      const consumptionValues: ConsumptionInterface[] = [];
      ...
      for (let i = 0; i < allIndexes.length - 1; i++) {
        ...
        const dailyConsumption: number =
          (nextIndex.indexValue - currentIndex.indexValue) / daysBetween;

        //* Günlük tüketim değerlerini tarihlere göre oluştur ve diziye ekle
        for (let j = 0; j < daysBetween; j++) {
          const consumptionDate: string = moment(currentIndex.date)
            .add(j, 'days')
            .format('YYYY-MM-DD');
          ...
          } as ConsumptionInterface);
        }
      }

      // 1. Seçenek
      // * Mevcut tüketim listesinde olmayan tarihleri sil
      const existingDates: string[] = consumptionValues.map((value) => value.date);
      ...
      await ConsumptionModel.bulkWrite(
        consumptionValues.map((value: ConsumptionInterface) => ({
          updateOne: {
